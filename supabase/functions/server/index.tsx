...
// AI Router - call appropriate model
async function callAIRouter(message: string, context: any, apiKey: string, tier: 'cheap' | 'smart'): Promise<string> {
  // Determine API URL and model based on tier
  let apiUrl: string;
  let model: string;

  const deepseekKey = Deno.env.get('DEEPSEEK_API_KEY');
  
  if (tier === 'cheap' && deepseekKey) {
    apiUrl = 'https://api.deepseek.com/v1/chat/completions';
    model = 'deepseek-chat';
    apiKey = deepseekKey;
  } else if (Deno.env.get('GROK_API_KEY')) {
    apiUrl = 'https://api.x.ai/v1/chat/completions';
    model = tier === 'smart' ? 'grok-2' : 'grok-2-mini';
    apiKey = Deno.env.get('GROK_API_KEY')!;
  } else {
    apiUrl = 'https://api.openai.com/v1/chat/completions';
    model = tier === 'smart' ? 'gpt-4o-mini' : 'gpt-3.5-turbo';
  }
...
- Top danh mục: ${(context.topCategories || []).map((c: any) => `${c.name}: ${c.amount?.toLocaleString('vi-VN')}`).join(', ')}`;
  ...
  throw new Error(`AI response empty: ${JSON.stringify(data)}`);
}

// Real OCR via AI Vision API
async function performRealOCR(imageBase64: string, apiKey: string): Promise<any> {
  ...
  const response = await fetch(apiUrl, {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: [
          { type: 'text', text: 'Extract from this receipt: amount (number), date (YYYY-MM-DD), merchant name, and suggest a Vietnamese expense category. Return JSON only: {"amount":number,"date":"string","merchantName":"string","category":"string","confidence":number}' },
